...
test("Test to GET Conversations from backend", () => {
  ...
  expect(mockAxios.get).toHaveReturnedWith(expect.objectContaining({
    data: conversation
  }));
});
...
test("Test to POST Message to backend", () => {
  ...
  expect(mockAxios.post).toHaveBeenCalledWith("https://speedy-recovery-server.azurewebsites.net/messages?", data, { headers });

  mockAxios.mockResponse();
  expect(mockAxios.post).toHaveReturnedTimes(1);

  expect(mockAxios.post).toHaveReturnedWith(expect.objectContaining({
