...
it("data converted properly from json to calendar format", () => {
  ...
  expect(wrapper.state().events[0].title).toEqual(
  ...
  expect(wrapper.state().events[0].description).toEqual(
    "Discussion on the results of your recent MRI"
  );
  expect(wrapper.state().events[0].start).toEqual(
    new Date("2019-01-01T09:00:00Z")
  );
  expect(wrapper.state().events[0].end).toEqual(
    new Date("2019-01-01T11:00:00Z")
  );
  expect(wrapper.state().events[0].comment).toEqual(
  ...
  expect(wrapper.state().events[0].location).toEqual(
  ...
  expect(wrapper.state().events[1].description).toEqual(
    "Discussion on the results of your recent CT"
  );
  expect(wrapper.state().events[1].start).toEqual(
    new Date("2019-01-15T11:00:00Z")
  );
  expect(wrapper.state().events[1].end).toEqual(
    new Date("2019-01-15T13:00:00Z")
  );
  expect(wrapper.state().events[1].comment).toEqual(
    "Further expand on the results of the CT and determine the " +
