  ...
  render() {
    ...
    const createMedicationDispenseTable = () => {
        ...
        if (medicationDispensesNum) {
            const header = [
                <Table.Row key={"medicationDRow"}>
                    {isBrowser || isTablet ? <Table.HeaderCell id="practitionerTableCell">#</Table.HeaderCell> : null}
                    <Table.HeaderCell id="practitionerTableCell">Medication</Table.HeaderCell>
                    {isBrowser || isTablet ?
                        <Table.HeaderCell id="practitionerTableCell">Search</Table.HeaderCell> : null}
                    {isBrowser || isTablet ?
                        <Table.HeaderCell id="practitionerTableCell">Quantity</Table.HeaderCell> : null}
                    {isBrowser ? <Table.HeaderCell id="practitionerTableCell">Days Supply</Table.HeaderCell> : null}
                    {isBrowser ? <Table.HeaderCell id="practitionerTableCell">Status</Table.HeaderCell> : null}
                    {isBrowser || isTablet ?
                        <Table.HeaderCell id="practitionerTableCell">Intake Form</Table.HeaderCell> : null}
                    {isBrowser || isTablet ?
                        <Table.HeaderCell id="practitionerTableCell">Intake Number</Table.HeaderCell> : null}
                </Table.Row>
            ];
            table.push(
        ...
        for (let i = 0; i < medicationDispensesNum; i++) {
            ...
            children.push(
                <Table.Cell key={"medicationSearchCell" + i} id="practitionerTableCell">
                    {
                        <h4>
            ...
            if (isBrowser || isTablet) {
                children.push(
                    <Table.Cell key={"medicationQuantityCell" + i} id="practitionerTableCell">
                        {<h4>{medicationDispenses[i].quantity}</h4>}
                    </Table.Cell>
                );
                if (!isTablet) {
                    children.push(
                        <Table.Cell key={"medicationSupplyCell" + i} id="practitionerTableCell">
                            {<h4>{medicationDispenses[i].daysSupply}</h4>}
                        </Table.Cell>
                    );
                    children.push(
                        <Table.Cell key={"medicationStatusCell" + i} id="practitionerTableCell">
                ...
                children.push(
                    <Table.Cell key={"medicationTimeCell" + i} id="practitionerTableCell">
                        {
                            <h4>
