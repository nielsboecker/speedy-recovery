...
/* This file defines the ParentCarePlanePane which creates a pane used in the ParentInfo component which displays all
care plan information regarding the Parent's child
 */
...
class ParentCarePlanPane extends Component {
  render() {
    ...
    const createCarePlanTable = () => {
      ...
      for (let i = 0; i < carePlansNum; i++) {
        ...
        children.push(
          <Table.Cell key={"carePlanOrderCell" + i}>
            <Label color = "orange" ribbon>{`${i + 1}`} </Label>
          </Table.Cell>
        );
        children.push(
      ...
      const carePlanTable = [];
      carePlanTable.push(<Table key="carePlanTable" color="orange">{table}</Table>);
    ...
    return (
      <div>
        <Tab.Pane>
          <h4>You have {carePlansNum} recorded care plan for {childName[0]}.</h4>
          {createCarePlanTable()}
        </Tab.Pane>
      </div>
    );
...
export default ParentCarePlanPane;