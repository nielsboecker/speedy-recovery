...
class ProfilePage extends Component {
    ...
    render() {
        const roleText = this.state.user.role === 'Patient'
            ? <Fragment><Icon name="user"/>Patient</Fragment>
    ...
    convertData() {
        ...
        const user = {
            ...
            language: dataFromFhir.communication.find(element => element.preferred).language.text,
            phone: dataFromFhir.telecom.filter(element => element.system === 'phone'),
            email: dataFromFhir.telecom.find(element => element.system === 'email').value
        };
