...
const mapPatientToUser = fhirPatientResource => ({
  ...
  birthDate: fhirPatientResource.birthDate ? new Date(fhirPatientResource.birthDate) : "Unknown",
...
const getGP = generalPractitioner => {
  if(generalPractitioner && generalPractitioner[0] && generalPractitioner[0].reference){
...
const getAddress = address => {
  if(address && address[0] && address[0].line + address[0].line[0] && address[0].city && address[0].state 
  && address[0].postalCode && address[0].country){
     return address[0].line[0] + ", " +  address[0].city + ", " + address[0].state + ", " 
     + address[0].postalCode + ", " +  address[0].country;
  }
  return "Unknown";
};
...
const mapAppointment = fhirAppResource => ({
  ...
  description: fhirAppResource.description ? fhirAppResource.description : "Unknown",
  ...
  created: fhirAppResource.created ? new Date(fhirAppResource.created) : "Unknown",
...
const getSupportingInfo = supportingInformation => {
  if(supportingInformation && supportingInformation[0] && supportingInformation[0].reference){
...
const getPatient = participant => {
  if(participant && participant[0] && participant[0].actor && participant[0].actor.display){
...
const getPractitioner = participant => {
  if(participant && participant[1] && participant[1].actor && participant[1].actor.display){
...
const getLocation = participant => {
  if(participant && participant[2] && participant[2].actor && participant[2].actor.display){
...
const mapCondition = fhirCondResource => ({
  clinicalStatus: fhirCondResource.clinicalStatus ? fhirCondResource.clinicalStatus: "Unknown",
  verificationStatus: fhirCondResource.verificationStatus ? fhirCondResource.verificationStatus: "Unknown",
  ...
  onsetDateTime: fhirCondResource.onsetDateTime? new Date(fhirCondResource.onsetDateTime) : "Unknown"
});

const getSeverity = severity => {
  if(severity && severity.coding && severity.coding[0] && severity.coding[0].display){
...
const mapMedication = fhirMedResource => ({
  ...
  isBrand: typeof(fhirMedResource.isBrand) !== "undefined" ? fhirMedResource.isBrand : "Unknown",
  isOverTheCounter: typeof(fhirMedResource.isOverTheCounter) !== "undefined" ? fhirMedResource.isOverTheCounter : "Unknown",
...
const getContent = packageC => {
  if(packageC && packageC.content && packageC.content[0] && packageC.content[0].itemCodeableConcept && 
    packageC.content[0].itemCodeableConcept.coding && packageC.content[0].itemCodeableConcept.coding[0] &&
